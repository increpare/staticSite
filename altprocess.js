...
for (var i=0;i<table.length;i++){
	...
	if (icon==""){
		...
		execAsync(`./generateicon.js ${safeName} output/icos/${icon}; ${convertcmd}; pngcrush -ow output/thumbs/${icon}`)
		r[2] = icon
	} else {
		...
		execAsync(`cp icos/${icon} output/icos/${icon} ; pngcrush -ow output/icos/${icon}; ${convertcmd}`)				
	}
	...
	var linkList = function(pre,presrc,post){
		...
		if (unityplayer!=""){
			result += `${pre} <a href="${flash}">Play Now</a> ${post} (Unity Web Player) \n`
		}
		if (src!=""){
...
function doGrid(){
	...
	for (var i=0;i<table.length;i++){
		...
		s+=`\t\t<td><a href="game/${pageName}"><img class="ico" width="100" height="100" src="thumbs/${icon}"><br>${title}</a>`
